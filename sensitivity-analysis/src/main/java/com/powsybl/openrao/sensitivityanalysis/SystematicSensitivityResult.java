        ...
        for (var e : instantOrderByState.entrySet()) {
            ...
            if (sensiState.contingencyId() == null) {
                ...
                SensitivityAnalysisResult.Status status = sensiState.operatorStrategyId() == null
                        ...
                        : getStateStatusOrFallback(statusByState, sensiState);
                ...
                // A state declared to the sensitivity provider does not always come back: when an operator strategy is
                // declared for all contingencies, the provider skips the ones it could not simulate (e.g. a contingency
                // on an already open branch). Falling back on the status of the contingency itself, and on FAILURE when
                // even that is missing, keeps a missing state from turning into a NullPointerException.

...
class ContingenciesCreationTest {
    ...
    void importContingencies() {
        ...
        assertEquals(1, cracCreationContext.getContingencyCreationContexts().stream().filter(StandardElementaryCreationContext::isAltered).toList().size());
        assertEquals("Incorrect contingent status for equipment(s): FFR1AA1  FFR3AA1  1, FFR1AA1  FFR4AA1  1. Missing contingent equipment(s) in network: missing-generator, missing-line.", cracCreationContext.getContingencyCreationContexts().stream().filter(StandardElementaryCreationContext::isAltered).toList().get(0).getImportStatusDetail());

                ...
                switch (jsonParser.getCurrentName()) {
                    ...
                    case TOPOLOGICAL_ACTIONS:
                        if (getPrimaryVersionNumber(version) > 2 || getPrimaryVersionNumber(version) == 2 && getSubVersionNumber(version) > 3) {
                            throw new OpenRaoException(String.format("%s is either %s or %s since CRAC version 2.3", TOPOLOGICAL_ACTIONS, TERMINALS_CONNECTION_ACTIONS, SWITCH_ACTIONS));
                        } else {
                            jsonParser.nextToken();
                    ...
                    case PST_SETPOINTS:
                        if (getPrimaryVersionNumber(version) > 2 || getPrimaryVersionNumber(version) == 2 && getSubVersionNumber(version) > 3) {
                            throw new OpenRaoException(String.format("%s is now %s since CRAC version 2.3", PST_SETPOINTS, PHASETAPCHANGER_TAPPOSITION_ACTIONS));
                        } else {
                            jsonParser.nextToken();
                    ...
                    case INJECTION_SETPOINTS:
                        if (getPrimaryVersionNumber(version) > 2 || getPrimaryVersionNumber(version) == 2 && getSubVersionNumber(version) > 3) {
                            throw new OpenRaoException(String.format("%s is either %s, or %s, or %s, or %s since CRAC version 2.3", INJECTION_SETPOINTS, GENERATOR_ACTIONS, LOAD_ACTIONS, DANGLINGLINE_ACTIONS, SHUNTCOMPENSATOR_POSITION_ACTIONS));
                        } else {
                            jsonParser.nextToken();

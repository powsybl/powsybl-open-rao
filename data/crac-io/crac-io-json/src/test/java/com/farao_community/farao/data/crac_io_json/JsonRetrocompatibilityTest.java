...
class JsonRetrocompatibilityTest {
    ...
    @Test
    void importCracRasWithSpeedTest() {
        InputStream cracFile = getClass().getResourceAsStream("/retrocompatibility/v1/crac-ras-with-speed.json");
        Crac crac = new JsonImport().importCrac(cracFile);

        assertEquals(123, crac.getPstRangeAction("pstRange1Id").getSpeed().get().intValue());
        assertEquals(200, crac.getHvdcRangeAction("hvdcRange1Id").getSpeed().get().intValue());
        assertEquals(10, crac.getNetworkAction("complexNetworkActionId").getSpeed().get().intValue());

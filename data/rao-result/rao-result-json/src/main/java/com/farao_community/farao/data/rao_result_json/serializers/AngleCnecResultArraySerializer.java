...
final class AngleCnecResultArraySerializer {
    ...
    private static void serializeAngleCnecResult(AngleCnec angleCnec, RaoResult raoResult, Crac crac, JsonGenerator jsonGenerator) throws IOException {
        ...
        if (containsAnyResultForAngleCnec(raoResult, angleCnec, crac)) {
            ...
            serializeAngleCnecResultForOptimizationState(crac.getInstant(Instant.Kind.PREVENTIVE), angleCnec, raoResult, jsonGenerator);

            if (!angleCnec.getState().isPreventive()) {
                serializeAngleCnecResultForOptimizationState(crac.getInstant(Instant.Kind.AUTO), angleCnec, raoResult, jsonGenerator);
                serializeAngleCnecResultForOptimizationState(crac.getInstant(Instant.Kind.CURATIVE), angleCnec, raoResult, jsonGenerator);
            }
    ...
    private static boolean containsAnyResultForAngleCnec(RaoResult raoResult, AngleCnec angleCnec, Crac crac) {
        ...
        if (angleCnec.getState().isPreventive()) {
            return containsAnyResultForOptimizationState(raoResult, angleCnec, null) ||
                containsAnyResultForOptimizationState(raoResult, angleCnec, crac.getInstant(Instant.Kind.PREVENTIVE));
        } else {
            return containsAnyResultForOptimizationState(raoResult, angleCnec, null) ||
                containsAnyResultForOptimizationState(raoResult, angleCnec, crac.getInstant(Instant.Kind.PREVENTIVE)) ||
                containsAnyResultForOptimizationState(raoResult, angleCnec, crac.getInstant(Instant.Kind.AUTO)) ||
                containsAnyResultForOptimizationState(raoResult, angleCnec, crac.getInstant(Instant.Kind.CURATIVE));
        }
    }

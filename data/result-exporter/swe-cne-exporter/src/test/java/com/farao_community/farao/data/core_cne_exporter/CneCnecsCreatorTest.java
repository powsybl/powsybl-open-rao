...
public class CneCnecsCreatorTest {
    ...
    public void setUp() {
        ...
        exporterParameters = new SweCneExporterParameters("22XCORESO------S-20211115-F299v1", 2, "10YDOM-REGION-1V", SweCneExporterParameters.ProcessType.DAY_AHEAD_CC,
            "22XCORESO------S", SweCneExporterParameters.RoleType.REGIONAL_SECURITY_COORDINATOR, "17XTSO-CS------W", SweCneExporterParameters.RoleType.CAPACITY_COORDINATOR, "2021-10-30T22:00Z/2021-10-31T23:00Z");
    }
    ...
    private void mockCnecResult(FlowCnec cnec, double flowA, double flowMw, double marginA, double marginMw, double relMarginA, double relMarginMw, double ptdf) {
        Mockito.when(raoResult.getFlow(any(), eq(cnec), eq(Unit.AMPERE))).thenReturn(flowA);
        Mockito.when(raoResult.getFlow(any(), eq(cnec), eq(Unit.MEGAWATT))).thenReturn(flowMw);
        ...
        Mockito.when(raoResult.getPtdfZonalSum(any(), eq(cnec))).thenReturn(ptdf);
    }
    ...
    public void testExportTwoPreventiveCnecs() {
        ...
        SweCneHelper cneHelper = new SweCneHelper(crac, network, new MockCracCreationContext(crac), raoResult, raoParameters, exporterParameters);
        SweCneCnecsCreator cneCnecsCreator = new SweCneCnecsCreator(cneHelper);
    ...
    public void testWithLoopFlow() {
        ...
        mockCnecResult(cnec1, 40, 80, 10, 20, 100, 200, .1);
        Mockito.when(raoResult.getLoopFlow(any(), eq(cnec1), eq(Unit.MEGAWATT))).thenReturn(123.);

...
public final class CoreCneCnecsCreator {
    ...
    private List<ConstraintSeries> createConstraintSeriesOfCnec(BranchCnecCreationContext branchCnecCreationContext, String outageCnecId, String curativeCnecId, boolean asMnec, CneHelper cneHelper) {
        ...
        if (optionalContingency.isPresent() &&
            (!cneHelper.getRaoResult().getActivatedNetworkActionsDuringState(cneHelper.getCrac().getState(optionalContingency.get(), cneHelper.getCrac().getInstant(Instant.Kind.CURATIVE))).isEmpty()
                || !cneHelper.getRaoResult().getActivatedRangeActionsDuringState(cneHelper.getCrac().getState(optionalContingency.get(), cneHelper.getCrac().getInstant(Instant.Kind.CURATIVE))).isEmpty())) {
            // B54

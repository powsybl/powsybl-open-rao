...
public final class OnContingencyStateUsageRuleHelper {
    ...
    public static Map<String, AssociationStatus> processContingenciesLinkedToRemedialAction(Crac crac,
        ...
        for (ContingencyWithRemedialAction nativeContingencyWithRemedialAction : linkedContingencyWithRemedialActions) {
            if (contingencyStatusMap.containsKey(nativeContingencyWithRemedialAction.contingency())) {
                contingencyStatusMap.put(
                    ...
                    new AssociationStatus(false, ("OnContingencyState usage rule for remedial action %s with contingency %s ignored because this contingency " +
                        "has several conflictual links to the remedial action.").formatted(remedialActionId, nativeContingencyWithRemedialAction.contingency()))
                );
                continue;
            }
            ...
            if (!nativeContingencyWithRemedialAction.normalEnabled()) {
                contingencyStatusMap.put(
                    nativeContingencyWithRemedialAction.contingency(),
                    new AssociationStatus(false, ("OnContingencyState usage rule for remedial action %s with contingency %s ignored because " +
                        "the association is disabled.").formatted(remedialActionId, nativeContingencyWithRemedialAction.contingency()))
                );
                continue;
            }
            ...
            if (!nativeContingencyWithRemedialAction.isIncluded()) {
                contingencyStatusMap.put(
                    nativeContingencyWithRemedialAction.contingency(),
                    new AssociationStatus(false, ("OnContingencyState usage rule for remedial action %s with contingency %s ignored because only included " +
                        "combinationConstraintKinds are supported.").formatted(remedialActionId, nativeContingencyWithRemedialAction.contingency()))
                );
                continue;
            }

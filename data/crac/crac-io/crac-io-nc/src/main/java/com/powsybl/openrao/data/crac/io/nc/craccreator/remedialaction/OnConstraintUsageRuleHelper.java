...
public final class OnConstraintUsageRuleHelper {
    ...
    public static Set<Cnec> getImportedCnecFromAssessedElementId(String assessedElementId, Crac crac, Set<ElementaryCreationContext> cnecCreationContexts) {
        return cnecCreationContexts.stream()
            .filter(context -> context.isImported() && assessedElementId.equals(context.getNativeObjectId()))
            .map(context -> crac.getCnec(context.getCreatedObjectId()))
            .collect(Collectors.toSet());
    }

    public static Set<Cnec> filterCnecsThatHaveGivenContingencies(Set<Cnec> cnecs, Set<String> contingenciesIds) {
        return cnecs.stream()
            .filter(cnec -> cnec.getState().getContingency().isPresent() && contingenciesIds.contains(cnec.getState().getContingency().get().getId()))
            .collect(Collectors.toSet());
    }

    public static Map<String, AssociationStatus> processCnecsLinkedToRemedialAction(Crac crac,
        ...
        Map<String, AssociationStatus> contingencyStatusMap = OnContingencyStateUsageRuleHelper.processContingenciesLinkedToRemedialAction(
            crac, remedialActionId, linkedContingencyWithRemedialActions);
        Map<String, AssociationStatus> cnecStatusMap = new HashMap<>();
        Set<String> validLinkedContingencies = contingencyStatusMap.entrySet().stream()
            .filter(entry -> entry.getValue().isValid()).map(Map.Entry::getKey)
            .collect(Collectors.toSet());
    ...
    private static boolean isAssociationInvalid(String remedialActionId,
                                                Map<String, AssociationStatus> cnecStatusMap,
                                                AssessedElementWithRemedialAction nativeAssessedElementWithRemedialAction,
                                                Set<Cnec> cnecs) {
        if (cnecStatusMap.containsKey(nativeAssessedElementWithRemedialAction.assessedElement()) || cnecs.stream().anyMatch(cnec -> cnecStatusMap.containsKey(cnec.getId()))) {
            cnecStatusMap.put(
                ...
                new AssociationStatus(
                    false,
                    "OnConstraint usage rule for remedial action %s with assessed element %s ignored because this assessed element has several conflictual links to the remedial action."
        ...
        if (!nativeAssessedElementWithRemedialAction.normalEnabled()) {
            cnecStatusMap.put(
                ...
                new AssociationStatus(
                    false,
                    "OnConstraint usage rule for remedial action %s with assessed element %s ignored because the association is disabled."
                        .formatted(remedialActionId, nativeAssessedElementWithRemedialAction.assessedElement())
                )
            );
            return true;
        ...
        if (!nativeAssessedElementWithRemedialAction.isIncluded()) {
            cnecStatusMap.put(
                ...
                new AssociationStatus(
                    false,
                    "OnConstraint usage rule for remedial action %s with assessed element %s ignored because only included combinationConstraintKinds are supported."
                        .formatted(remedialActionId, nativeAssessedElementWithRemedialAction.assessedElement())
                )
            );
            return true;

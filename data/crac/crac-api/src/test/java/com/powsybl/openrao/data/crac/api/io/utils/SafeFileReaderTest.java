...
class SafeFileReaderTest extends TestBase {
    ...
    void shouldReadCorrectly() {
        ...
        Assertions.assertEquals("a very small file, for testing only. EOF here",
            new String(result1));
        //and then
    ...
    void shouldHandleReadError() {
        ...
        //then
        var ex = Assertions.assertThrowsExactly(UncheckedIOException.class,
            () -> sfr.withReadStream(InputStream::readAllBytes));
        Assertions.assertEquals("Error opening read stream: unknown.txt", ex.getMessage());
    }

    @Test
    void shouldHandleReadRunError() {
        //when
        var file = getResourceAsFile("/com/powsybl/openrao/data/crac/api/io/utils/small_file.txt");
        var sfr = SafeFileReader.create(file, BufferSize.DEFAULT);
        //then
        var ex = Assertions.assertThrowsExactly(RunException.class,
            () -> sfr.withReadStreamVoid(is -> {

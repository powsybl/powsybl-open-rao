...
public class OnStateAdderToRemedialActionImpl<T extends AbstractRemedialAction<T>> implements OnContingencyStateAdderToRemedialAction<T> {
    ...
    public T add() {
        ...
        if (state.isPreventive() && usageMethod != UsageMethod.FORCED_IF_AVAILABLE) {
            throw new FaraoException("OnContingencyState usage rules are not allowed for PREVENTIVE instant except when FORCED_IF_AVAILABLE. Please use newOnInstantUsageRule() instead.");

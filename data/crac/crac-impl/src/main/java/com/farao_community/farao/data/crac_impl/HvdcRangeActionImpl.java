...
public class HvdcRangeActionImpl extends AbstractRangeAction<HvdcRangeAction> implements HvdcRangeAction {
    ...
    public double getMinAdmissibleSetpoint(double previousInstantSetPoint) {
        ...
        for (StandardRange range : ranges) {
            minAdmissibleSetpoint = switch (range.getRangeType()) {
                case ABSOLUTE -> Math.max(minAdmissibleSetpoint, range.getMin());
                case RELATIVE_TO_INITIAL_NETWORK -> Math.max(minAdmissibleSetpoint, initialSetpoint + range.getMin());
                case RELATIVE_TO_PREVIOUS_INSTANT ->
                    Math.max(minAdmissibleSetpoint, previousInstantSetPoint + range.getMin());
            };
        }
    ...
    public double getMaxAdmissibleSetpoint(double previousInstantSetPoint) {
        ...
        for (StandardRange range : ranges) {
            maxAdmissibleSetpoint = switch (range.getRangeType()) {
                case ABSOLUTE -> Math.min(maxAdmissibleSetpoint, range.getMax());
                case RELATIVE_TO_INITIAL_NETWORK -> Math.min(maxAdmissibleSetpoint, initialSetpoint + range.getMax());
                case RELATIVE_TO_PREVIOUS_INSTANT ->
                    Math.min(maxAdmissibleSetpoint, previousInstantSetPoint + range.getMax());
            };
        }

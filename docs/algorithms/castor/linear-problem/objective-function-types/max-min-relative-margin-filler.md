...
| Highest threshold value  | $MaxRAM$                  | A "bigM" which is computed (by OpenRAO) as the greatest absolute possible value of the CNEC threshold, among all CNECs in the CRAC. <br> It represents the common greatest possible value for a given CNEC's margin (exception made of CNECs only constrained in one direction, but this value should be high enough not to have any effect on those). In  [flow's unit](../../../../parameters/business-parameters.md#objective-function-parameters) |
...
> 💡 Max Min Relative Margin constraints are considered in the same unit as the [flow's unit](../../../../parameters/business-parameters.md#objective-function-parameters) in the MIP.

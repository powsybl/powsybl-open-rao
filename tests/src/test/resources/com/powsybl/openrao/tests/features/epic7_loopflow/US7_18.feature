...
Feature: US 7.18: Virtual hubs in loopflow computation
  ...
  The expected result should match the Megawatt value converted to Ampere using the formula : flowInAmpere ~ flowInMw / (Unom × sqrt(3) / 1000).
    ...
    When I launch loopflow_computation with OpenLoadFlow at "2019-01-08 21:30"
    And the loopflow on cnec "BBE1AA1  BBE2AA1  1 - preventive" after loopflow computation should be -1312.0 A
    And the loopflow on cnec "BBE1AA1  BBE3AA1  1 - preventive" after loopflow computation should be -1140.0 A
    And the loopflow on cnec "FFR1AA1  FFR2AA1  1 - preventive" after loopflow computation should be 1477.0 A
    And the loopflow on cnec "FFR1AA1  FFR3AA1  1 - preventive" after loopflow computation should be -33.0 A
    And the loopflow on cnec "FFR2AA1  FFR3AA1  1 - preventive" after loopflow computation should be -1510.0 A
    And the loopflow on cnec "DDE1AA1  DDE2AA1  1 - preventive" after loopflow computation should be -254.0 A
    And the loopflow on cnec "DDE1AA1  DDE3AA1  1 - preventive" after loopflow computation should be -1187.0 A
    And the loopflow on cnec "DDE2AA1  DDE3AA1  1 - preventive" after loopflow computation should be -932.0 A
    And the loopflow on cnec "NNL1AA1  NNL2AA1  1 - preventive" after loopflow computation should be -109.0 A
    And the loopflow on cnec "NNL1AA1  NNL3AA1  1 - preventive" after loopflow computation should be -466.0 A
    And the loopflow on cnec "NNL2AA1  NNL3AA1  1 - preventive" after loopflow computation should be -355.0 A
    And the loopflow on cnec "FFR2AA1  DDE3AA1  1 - preventive" after loopflow computation should be -764.0 A
    And the loopflow on cnec "DDE2AA1  NNL3AA1  1 - preventive" after loopflow computation should be -764.0 A
    And the loopflow on cnec "NNL2AA1  BBE3AA1  1 - preventive" after loopflow computation should be -476.0 A
    And the loopflow on cnec "BBE2AA1  FFR3AA1  1 - preventive" after loopflow computation should be -620.0 A
  ...
  The expected result should match the Megawatt value converted to Ampere using the formula : flowInAmpere ~ flowInMw / (Unom × sqrt(3) / 1000).
    ...
    Given configuration file is "common/RaoParameters_default_ampere.json"
    When I launch loopflow_computation with OpenLoadFlow at "2019-01-08 21:30"
    And the loopflow on cnec "BBE1AA1  BBE2AA1  1 - preventive" after loopflow computation should be -1343.0 A
    And the loopflow on cnec "BBE1AA1  BBE3AA1  1 - preventive" after loopflow computation should be -1300.0 A
    And the loopflow on cnec "FFR1AA1  FFR2AA1  1 - preventive" after loopflow computation should be 1573.0 A
    And the loopflow on cnec "FFR1AA1  FFR3AA1  1 - preventive" after loopflow computation should be 157.0 A
    And the loopflow on cnec "FFR2AA1  FFR3AA1  1 - preventive" after loopflow computation should be -1415.0 A
    And the loopflow on cnec "DDE1AA1  DDE2AA1  1 - preventive" after loopflow computation should be -157.0 A
    And the loopflow on cnec "DDE1AA1  DDE3AA1  1 - preventive" after loopflow computation should be -1284.0 A
    And the loopflow on cnec "DDE2AA1  DDE3AA1  1 - preventive" after loopflow computation should be -1127.0 A
    And the loopflow on cnec "NNL1AA1  NNL2AA1  1 - preventive" after loopflow computation should be 274.0 A
    And the loopflow on cnec "NNL1AA1  NNL3AA1  1 - preventive" after loopflow computation should be 446.0 A
    And the loopflow on cnec "NNL2AA1  NNL3AA1  1 - preventive" after loopflow computation should be 170.0 A
    And the loopflow on cnec "DDE2AA1  X_NLDE1  1 + NNL3AA1  X_NLDE1  1 - preventive" after loopflow computation should be -473.0 A
    And the loopflow on cnec "DDE3AA1  X_DEFR1  1 + FFR2AA1  X_DEFR1  1 - preventive" after loopflow computation should be 329.0 A
    And the loopflow on cnec "BBE3AA1  X_NLBE1  1 + NNL2AA1  X_NLBE1  1 - preventive" after loopflow computation should be 617.0 A
    And the loopflow on cnec "BBE2AA1  X_BEFR1  1 + FFR3AA1  X_BEFR1  1 - preventive" after loopflow computation should be -906.0 A

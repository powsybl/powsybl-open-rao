...
Feature: US 93.1: Redispatching actions
  ...
  Scenario: US 93.1.1.b: Basic redispatching minCost
    ...
    Then 1 remedial actions are used in preventive
    And the remedial action "redispatchingAction" is used in preventive
    And the setpoint of RangeAction "redispatchingAction" should be 530.0 MW in preventive
    And the margin on cnec "cnecFr1Fr3Preventive" after PRA should be 10.0 MW
    # Total cost : 10 for activation + 50 x 530 MW shift = 26510
    And the value of the objective function after PRA should be 26510.0

...
Feature: 5.1.2.5: Additional tests with CBCORA input files and FRM
  ...
  @fast @rao @dc @preventive-only @loopflow @max-min-margin
  Scenario: 5.1.2.5.2 : Loop-flow limitation with FRM
    Given network file is "common/TestCase12Nodes.uct" for CORE CC
    ...
    Then its security status should be "UNSECURED"
    Then the worst margin is -451.0 MW
    Then the worst margin is -451.0 MW on cnec "FFR2AA1  DDE3AA1  1 - preventive"
    ...
    Then the initial loopflow on cnec "DDE2AA1  NNL3AA1  1 - preventive" should be -391.0 MW
    Then the loopflow on cnec "DDE2AA1  NNL3AA1  1 - preventive" after PRA should be -446.0 MW
    Then the initial loopflow on cnec "NNL2AA1  BBE3AA1  1 - preventive" should be -391.0 MW
    Then the loopflow on cnec "NNL2AA1  BBE3AA1  1 - preventive" after PRA should be -446.0 MW
    Then the initial loopflow on cnec "FFR2AA1  DDE3AA1  1 - preventive" should be -391.0 MW
    Then the loopflow on cnec "FFR2AA1  DDE3AA1  1 - preventive" after PRA should be -446.0 MW
    Then the initial loopflow on cnec "BBE2AA1  FFR3AA1  1 - preventive" should be -391.0 MW
    Then the loopflow on cnec "BBE2AA1  FFR3AA1  1 - preventive" after PRA should be -446.0 MW
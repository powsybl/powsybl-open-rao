...
Feature: US 4.2: Computation in MAX_MIN_MARGIN: optimization in A/MW, thresholds in A/MW, computation in AC/DC
  ...
  Scenario: US 4.2.1.1: MW thresholds in DC mode and min margin in MW
    ...
    And the worst margin is 22 MW
    And the margin on cnec "FFR2AA1  DDE3AA1  1 - N-1 NL1-NL3 - curative" after PRA should be 22 MW
  ...
  Same data as US 4.2.1, but the computation is in AC.
    ...
    When I launch rao
    Then the execution details should be "The RAO only went through first preventive"
    And the worst margin is 18.0 MW
    And the value of the objective function after CRA should be -18.0
    And the margin on cnec "NNL2AA1  BBE3AA1  1 - preventive" after PRA should be 18.0 MW
  ...
  Same inputs as US 4.2.1.1, but the thresholds are defined in A in the CRAC.
    ...
    When I launch rao
    Then the execution details should be "The RAO only went through first preventive"
    And the worst margin is 15.07 MW
    And the value of the objective function after CRA should be -15.07
  ...
  Same data as US 4.2.2.1, but the computation is in AC.
    ...
    When I launch rao
    Then the execution details should be "The RAO only went through first preventive"
    And the worst margin is 19.0 A
  ...
  Same inputs as US 4.2.1.1, but some thresholds are defined in A in the CRAC (and others in MW).
    ...
    When I launch rao
    Then the execution details should be "The RAO only went through first preventive"
    And the worst margin is 18.52 MW
    And the value of the objective function after CRA should be -18.52
    And the tap of PstRangeAction "PRA_PST_BE" should be 4 in preventive
    And the margin on cnec "NNL2AA1  BBE3AA1  1 - preventive" after PRA should be 18.52 MW
    And the margin on cnec "FFR2AA1  DDE3AA1  1 - N-1 NL1-NL3 - outage" after PRA should be 23.38 MW

  @fast @rao @ac @preventive-only @max-min-margin @megawatt
  Scenario: US 4.2.3.2: mixed thresholds in AC mode and min margin in MW
  Same data as US 4.2.3.1, but the computation is in AC.
    ...
    When I launch rao
    Then the execution details should be "The RAO only went through first preventive"
    And the worst margin is 19.0 MW
    And the value of the objective function after CRA should be -19.0
    And the tap of PstRangeAction "PRA_PST_BE" should be 4 in preventive
    And the margin on cnec "NNL2AA1  BBE3AA1  1 - preventive" after PRA should be 19.0 MW

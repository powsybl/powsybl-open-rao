...
Feature: US 4.2: Optimization in A/MW, thresholds in A/MW, computation in AC/DC
  ...
  Scenario: US 4.2.6: mixed thresholds in AC mode and min margin in A
    ...
    Then the worst margin is 67.0 A
    Then the value of the objective function after CRA should be -67.0
    And the margin on cnec "NNL2AA1  BBE3AA1  1 - preventive" after PRA should be 88.0 A
    And the margin on cnec "FFR2AA1  DDE3AA1  1 - N-1 NL1-NL3 - outage" after PRA should be 76.0 A
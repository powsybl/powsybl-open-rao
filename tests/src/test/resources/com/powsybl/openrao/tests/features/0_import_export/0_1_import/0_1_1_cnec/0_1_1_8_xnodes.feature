...
Feature: 0.1.1.8: Handle X-nodes
  This feature covers CRAC import and RAO computation on a network containing an X-node.

  @fast @crac
  Scenario: 0.1.1.8.1: Handle CRAC import with half-line IDs
  No threshold with a PERCENT_IMAX unit is defined in the CRAC so no iMax was imported.
    ...
    Then it should have the following flow CNECs:
      ...
      | Cnec BE-FR | BBE2AA1  X_BEFR1  1 + FFR3AA1  X_BEFR1  1 | preventive |             | yes       | no        | NaN      | NaN       | 380                | 380                 |
      | Cnec BE-FR | BBE2AA1  X_BEFR1  1 + FFR3AA1  X_BEFR1  1 | outage     | N-1 DE-NL   | yes       | no        | NaN      | NaN       | 380                | 380                 |
      | Cnec DE-FR | DDE3AA1  X_DEFR1  1 + FFR2AA1  X_DEFR1  1 | preventive |             | yes       | no        | NaN      | NaN       | 380                | 380                 |
      | Cnec DE-FR | DDE3AA1  X_DEFR1  1 + FFR2AA1  X_DEFR1  1 | outage     | N-1 DE-NL   | yes       | no        | NaN      | NaN       | 380                | 380                 |
  ...
  @fast @rao @ac @preventive-only @max-min-margin
  Scenario: 0.1.1.8.3: Run a rao on a simple network with one X-node but the line containing X-node is not optimized
  ...
  @fast @rao @ac @preventive-only @max-min-margin
  Scenario: 0.1.1.8.4: Run a rao on a simple network with one X-node and the line containing X-node is optimized
    Given network file is "crac7/TestCase12Nodes_with_one_Xnode.uct"
  ...
  @fast @rao @ac @preventive-only @max-min-margin
  Scenario: 0.1.1.8.5: Run a rao on a network with X-nodes at each border and 2 lines are optimized
    Given network file is "crac7/TestCase12Nodes_with_Xnodes.uct"
    Given crac file is "crac7/ls-Xnodes.json"
    Given configuration file is "common/RaoParameters_maxMargin_ampere.json"
    When I launch rao

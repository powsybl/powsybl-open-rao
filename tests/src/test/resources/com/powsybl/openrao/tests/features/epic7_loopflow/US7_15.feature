...
Feature: US 7.15: loop-flow acceptable augmentation parameter
  ...
  Scenario: 7.15.1 : Test case with a loop-flow acceptable augmentation parameter of 40 MW
    ...
    Given configuration file is "epic7/RaoParameters_maxMargin_ampere_ac_lf_false_5_100_lfAugmentation.json"
    When I launch loopflow search_tree_rao at "2019-01-08 12:00" with default loopflow limit as 0 percent of pmax
    Then its security status should be "UNSECURED"
    Then the worst margin is -611 A
    Then the worst margin is -611 A on cnec "FFR2AA1  DDE3AA1  1 - preventive"
    ...
    And the loopflow on cnec "DDE2AA1  NNL3AA1  1 - preventive" after PRA should be -425.0 MW
    And the initial loopflow on cnec "NNL2AA1  BBE3AA1  1 - preventive" should be -391.0 MW
    And the loopflow on cnec "NNL2AA1  BBE3AA1  1 - preventive" after PRA should be -425.0 MW
    And the initial loopflow on cnec "FFR2AA1  DDE3AA1  1 - preventive" should be -391.0 MW
    And the loopflow on cnec "FFR2AA1  DDE3AA1  1 - preventive" after PRA should be -425.0 MW
    And the initial loopflow on cnec "BBE2AA1  FFR3AA1  1 - preventive" should be -391.0 MW
    And the loopflow on cnec "BBE2AA1  FFR3AA1  1 - preventive" after PRA should be -425.0 MW
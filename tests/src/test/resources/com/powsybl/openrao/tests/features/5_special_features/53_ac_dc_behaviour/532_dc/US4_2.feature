...
Feature: US 4.2: Computation in MAX_MIN_MARGIN: optimization in A/MW, thresholds in A/MW, computation in AC/DC
  ...
  Same data as US 4.2.1, but the computation is in AC.
    ...
    Then the margin on cnec "NNL2AA1  BBE3AA1  1 - preventive" after PRA should be 18.0 MW
    Then the tap of PstRangeAction "PRA_PST_BE" should be 4 in preventive
    Then the margin on cnec "FFR2AA1  DDE3AA1  1 - N-1 NL1-NL3 - outage" after PRA should be 28.1 MW
    Then the margin on cnec "NNL2AA1  BBE3AA1  1 - preventive" after PRA should be 19.0 MW
    Then the margin on cnec "FFR2AA1  DDE3AA1  1 - preventive" after PRA should be 49.0 MW
  ...
  Same data as US 4.2.3.1, but the computation is in AC.
    ...
    Then the tap of PstRangeAction "PRA_PST_BE" should be 4 in preventive
    Then the margin on cnec "NNL2AA1  BBE3AA1  1 - preventive" after PRA should be 19.0 MW
    Then the margin on cnec "FFR2AA1  DDE3AA1  1 - N-1 NL1-NL3 - outage" after PRA should be 23.0 MW
...
Feature: 5.2.2: Handle MNECs in search tree with only network actions
  ...
  Scenario: 5.2.2.3: margin on MNEC should stay above initial value - 260 A
    Given network file is "common/TestCase12Nodes.uct" for CORE CC
    ...
    Then 1 remedial actions are used in preventive
    Then the worst margin is -472.74 A on cnec "FFR2AA1  DDE3AA1  1 - preventive"
    ...
    Then the margin on cnec "NNL2AA1  BBE3AA1  1 - preventive" after PRA should be -975.0 A
    Then the flow on cnec "NNL2AA1  BBE3AA1  1 - preventive" after PRA should be -2418.0 A on side 1
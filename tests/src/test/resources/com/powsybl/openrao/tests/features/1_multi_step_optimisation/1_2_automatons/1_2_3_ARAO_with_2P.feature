...
Feature: 1.2.3: ARAO with 2P
  This feature covers the inclusion of ARAs in the second preventive computation.

  @fast @rao @ac @second-preventive @mnec
  Scenario: 1.2.3.1: Check that PRAO2 ignores applied ARAs from ARAO1
    ...
    Then the worst margin is -206 A
    Then the value of the objective function after CRA should be 206.1
    Then the margin on cnec "FR2-FR3-O - preventive" after PRA should be -206.1 A
    Then the margin on cnec "NL2-BE3-O - curative" after CRA should be 2.6 A
    Then the execution details should be "Second preventive improved first preventive results"

  @fast @rao @ac @second-preventive @mnec
  Scenario: 1.2.3.2: ARAO2
    Given network file is "common/TestCase12Nodes2PSTs.uct" for CORE CC
    ...
    Then the worst margin is -206 A
    Then the value of the objective function after CRA should be 206
    Then the margin on cnec "NL2-BE3-O - curative" after CRA should be 7.44 A
    Then the execution details should be "Second preventive improved first preventive results"
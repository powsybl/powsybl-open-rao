...
Feature: 3.1.1: Optimize computations for SECURE_FLOW objective function
  This feature covers the objective-function/type SECURE_FLOW.

  @fast @rao @dc @preventive-only @search-tree-rao @secure-flow
  Scenario: 3.1.1.1: Interrupt search tree depth early
  ...
  @fast @rao @dc @preventive-only @search-tree-rao @secure-flow
  Scenario: 3.1.1.2: Interrupt search tree depth early two threads
    Given network file is "epic91/TestCase4Nodes.uct"
    Given crac file is "epic91/CBCORA_interrupt_search_tree.xml"
  ...
  Scenario: 3.1.1.3: Skip curative optimization
    ...
    Then 0 remedial actions are used after "Contingency" at "curative"
    Then the worst margin is -70.9 MW
    # Check that curative CNECs' results are available
    Then the margin on cnec "CnecCurativeDir - curative" after PRA should be 1359.6 MW
    Then the margin on cnec "CnecCurativeDir - curative" after ARA should be 1359.6 MW
    Then the margin on cnec "CnecCurativeDir - curative" after CRA should be 1359.6 MW
    Then the margin on cnec "CnecCurativeOppo - curative" after PRA should be 1931.3 MW
    Then the margin on cnec "CnecCurativeOppo - curative" after ARA should be 1931.3 MW
    Then the margin on cnec "CnecCurativeOppo - curative" after CRA should be 1931.3 MW

        ...
        Set<RemedialAction<?>> availableRemedialActions =
            crac.getRemedialActions().stream()
                .filter(remedialAction ->
                    remedialAction.getTriggerConditions().stream().filter(tc -> tc.getContingency().isEmpty() && tc.getCnec().isPresent() && tc.getCountry().isEmpty() && tc.getCnec().get() instanceof VoltageCnec)
                        .anyMatch(onVoltageConstraint -> onVoltageConstraint.getCnec().get().equals(voltageCnec)))

...
public final class OpenTelemetryReporter {
    ...
    /**
     * Initializes the OpenTelemetry SDK with a simple logging exporter. In a production
     * environment, you would use an exporter like OTLP to send data to a backend (e.g., Jaeger,
     * Prometheus, etc.).
     *
     * @param endPoint the Open Telemetry trace collector (ex: '<a
     *                 href="http://localhost:4317">...</a>')
    ...
    public static void setOpenTelemetryTracer(SdkTracerProvider tracerProvider,
        boolean traceAllLogs) {
        TRACE_ALL_LOGS = traceAllLogs;
        var hasProvider = tracerProvider != null;
        TRACER =
            hasProvider ? tracerProvider.get(OPEN_RAO) : GlobalOpenTelemetry.getTracer(OPEN_RAO);
    }
        ...
        return new SpanExporter() {
            ...
            public CompletableResultCode export(Collection<SpanData> collection) {
                for (SpanData spanData : collection) {
                    LOGGER.info("[{}] >> {}\t{}\t{}\t{} ms", spanData.getParentSpanId(),
                        spanData.getSpanId(), spanData.getName(), spanData.getStatus(), Math.ceil(
                            (double) (spanData.getEndEpochNanos() - spanData.getStartEpochNanos())
                                / 1000000));
                }
                return null;
            }
    ...
    /**
     * A generic utility method to wrap a method call with a span. This method handles span
     * creation, context management, and error handling.
     *
     ...
     */
    public static <T> T withSpan(String spanName,
        ThrowingFunction<OpenTelemetryContext, T> runnable) {
    ...
    protected static <T> T doWithSpan(String spanName,
        ThrowingFunction<OpenTelemetryContext, T> callable) {
    ...
    /**
     * Runs the given task in an existing Open Telemetry Context, i.e. withSpan() has been called
     * before This is necessary when running tasks in different Threads (e.g. using ForkJoinTask)
     *
     * @param cx
     * @param task
     */
    ...
    private static void doLog(boolean loggerEnabled, Consumer<String> logWriter, String format,
        Object... arguments) {
        if (!loggerEnabled) {
            return;
        }

...
public class LoadFlowServiceTest {
    ...
    public void testLoadFlowService() {
        ...
        when(runner.run(any(Network.class), anyString(), any(ComputationManager.class), any(LoadFlowParameters.class))).thenReturn(loadFlowResult);
    ...
    public void testExceptionInLoadFlowService() {
        LoadFlow.Runner runner = Mockito.mock(LoadFlow.Runner.class);
        when(runner.run(any(Network.class), anyString(), any(ComputationManager.class), any(LoadFlowParameters.class))).thenThrow(new PowsyblException());

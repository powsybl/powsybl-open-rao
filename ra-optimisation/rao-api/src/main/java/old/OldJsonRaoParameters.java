 ...
 */
public final class OldJsonRaoParameters {
     ...
     */
    public interface OldExtensionSerializer<E extends Extension<OldRaoParameters>> extends ExtensionJsonSerializer<OldRaoParameters, E> {
    }

    /**
     * Lazily initialized list of extension serializers.
     */
    private static final Supplier<ExtensionProviders<OldExtensionSerializer>> SUPPLIER =
            Suppliers.memoize(() -> ExtensionProviders.createProvider(OldExtensionSerializer.class, "rao-parameters"));

    /**
     * @return the known extension serializers.
     */
    public static ExtensionProviders<OldExtensionSerializer> getExtensionSerializers() {
        return SUPPLIER.get();
    }

    private OldJsonRaoParameters() {
    }
     ...
     */
    public static OldRaoParameters read(Path jsonFile) {
        return update(new OldRaoParameters(), jsonFile);
    }
     ...
     * @return  parameters from a JSON file (will NOT rely on platform config).
     */
    public static OldRaoParameters read(InputStream jsonStream) {
        return update(new OldRaoParameters(), jsonStream);
    }
     ...
     */
    public static OldRaoParameters update(OldRaoParameters parameters, Path jsonFile) {
     ...
     */
    public static OldRaoParameters update(OldRaoParameters parameters, InputStream jsonStream) {
        try {
     ...
     */
    public static void write(OldRaoParameters parameters, Path jsonFile) {
        Objects.requireNonNull(jsonFile);
     ...
     */
    public static void write(OldRaoParameters parameters, OutputStream outputStream) {
        try {
            ObjectMapper objectMapper = createObjectMapper();
     ...
     */
    public static OldRaoParameters deserialize(JsonParser parser, DeserializationContext context, OldRaoParameters parameters) throws IOException {
        return new OldRaoParametersDeserializer().deserialize(parser, context, parameters);
    }
     ...
     * @throws IOException when an unexpected field is found
     */
    public static OldRaoParameters deserialize(JsonParser parser, DeserializationContext context) throws IOException {
        return new OldRaoParametersDeserializer().deserialize(parser, context);
    }
     ...
     */
    public static void serialize(OldRaoParameters parameters, JsonGenerator jsonGenerator, SerializerProvider serializerProvider) throws IOException {
        new OldRaoParametersSerializer().serialize(parameters, jsonGenerator, serializerProvider);
    }

    private static ObjectMapper createObjectMapper() {
        return JsonUtil.createObjectMapper()
                .registerModule(new OldRaoParametersJsonModule())

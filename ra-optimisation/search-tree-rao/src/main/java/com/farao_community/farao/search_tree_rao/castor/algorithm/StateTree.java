...
public class StateTree {
    ...
    private static boolean anyAvailableRemedialAction(Crac crac, State state) {
        return !crac.getNetworkActions(state, UsageMethod.AVAILABLE, UsageMethod.TO_BE_EVALUATED, UsageMethod.FORCED_IF_AVAILABLE).isEmpty() ||
                !crac.getRangeActions(state, UsageMethod.AVAILABLE, UsageMethod.TO_BE_EVALUATED, UsageMethod.FORCED_IF_AVAILABLE).isEmpty();
    }
    ...
    static boolean tsoHasCra(String tso, Crac crac, Set<State> optimizedCurativeStates) {
        return optimizedCurativeStates.stream().anyMatch(state ->
           crac.getNetworkActions(state, UsageMethod.AVAILABLE, UsageMethod.TO_BE_EVALUATED, UsageMethod.FORCED_IF_AVAILABLE).stream().map(RemedialAction::getOperator).anyMatch(raTso -> raTso.equals(tso)) ||
                crac.getRangeActions(state, UsageMethod.AVAILABLE, UsageMethod.TO_BE_EVALUATED, UsageMethod.FORCED_IF_AVAILABLE).stream().map(RemedialAction::getOperator).anyMatch(raTso -> raTso.equals(tso))
        );
    }
}

...
public class MinMarginEvaluator implements CostEvaluator {
    ...
    private double getMinMarginInMegawatt(RaoData raoData) {
        return raoData.getCrac().getCnecs().stream().filter(Cnec::isOptimized).collect(Collectors.toList()).stream().
    ...
    private double getMinMarginInAmpere(RaoData raoData) {
        List<Double> marginsInAmpere = raoData.getCrac().getCnecs().stream().filter(Cnec::isOptimized).collect(Collectors.toList()).stream().
    ...
    private List<Double> getMarginsInAmpereFromMegawattConversion(RaoData raoData) {
        return raoData.getCrac().getCnecs().stream().filter(Cnec::isOptimized).collect(Collectors.toList()).stream().map(cnec -> {
